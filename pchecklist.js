...
var passwordCheckOld = function () {
  ...
  var flag;  // define a flag 
  if (stringPassword.length < 8) {       // validation if the password length is less than 8 defined it was true. 
    flag = true;   
    stringOutput = 'password less than 8'; // if the passward less than 8 output "password less then 8"
  } else{                                                  // else loop "arrayBadPasswords" this arry  
    for (var i = 0; i < arrayBadPasswords.length; i++) {
      if (stringPassword === arrayBadPasswords[i]) {
        flag = true;   // if your input equal " arrayBadPasswords"  flag true output ' bad password ' 
    ...
    if (!flag) {
      stringOutput = 'great password'; // when flag equal false is a great password 
    }
  }
